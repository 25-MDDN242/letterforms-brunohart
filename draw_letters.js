...
// figuring out how tf to do this was very hard, I dont know why I spent so long doing this but I wanted to try and create the effect that Ryan had with the addition of the kind of idea of a coffee drop splash. 
function interpolate_letter(percent, oldBean, newBean) {
  ...
  // 2. Define when our big crema blob ripples: from 20% → 80%
  const splashStart = 20;
  const splashEnd   = 80;
  ...
  // 5. Now blend our bean parameters in three stages:
  if (percent < splashStart) {
    // → 0%→20%: Old bean → mid bean
    let t0 = percent / splashStart;
    for (let prop in mid) {
      out[prop] = lerp(oldBean[prop], mid[prop], t0);
    }

  } else if (percent <= splashEnd) {
    // → 20%→80%: Hold mid bean + splash blob on all three pours
    Object.assign(out, mid);

    ['foamHeight','milkBody'].forEach(group => {
      for (let i = 1; i <= 3; i++) {
        // foamHeight swells big, milkBody just a little extra body
        const boost = (group === 'foamHeight') ? 150 : 30;
    ...
    // → 80%→100%: mid bean → new bean
    let t1 = (percent - splashEnd) / (100 - splashEnd);
    for (let prop in mid) {
